...
namespace helix{namespace unittest{
	...
	public:
		...
		int run()
		{
			std::cout << "Lemon unittest console runner, version " 
				<< HELIX_VERSION.major << "." << HELIX_VERSION.minor 
				<< "." << HELIX_VERSION.build << "." << HELIX_VERSION.reversion << std::endl;
...
#define HELIX_UNITTEST_CASE(TestSuiteName,TestCaseName)\
class TestCaseName : private helix::nocopyable\
{\
public:\
...
namespace{\
	helix::unittest::test_case_factory<TestCaseName> __object_##TestCaseName##_factory(#TestSuiteName,#TestCaseName);\
}\
void TestCaseName::run()


#define helix_check(condition) helix::unittest::check((bool)(condition),"condition check error --> "#condition,__FILE__,__LINE__)

#define helix_expect_exception(exp,exception) try{(exp);helix::unittest::check(false,"expect exception failed"#exception,__FILE__,__LINE__);}catch(const exception&){}
