	...
	public:
		...
		int run()
		{
			std::cout << "Lemon unittest console runner, version " 
				<< LEMON_VERSION.major << "." << LEMON_VERSION.minor 
				<< "." << LEMON_VERSION.build << "." << LEMON_VERSION.reversion << std::endl;
...
namespace{\
	lemon::unittest::test_case_factory<TestCaseName> __object_##TestCaseName##_factory(#TestSuiteName,#TestCaseName);\
}\
