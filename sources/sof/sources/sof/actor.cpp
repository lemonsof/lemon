...
# endif
	...
	void __f(sof_actor * self)
	{
		assert(self->Context);

		try
		{
			self->F((sof_state)self,self->Userdata);
		}
		catch(...)
		{
		...
		sof_context_jump(self->Context,self->Q->context(),0);
	}

	sof_actor::sof_actor()
		:Id(SOF_MAIN_ACTOR_ID)
	...
	sof_actor::sof_actor(sof_t id,sof_f f, void * userdata,sof_context_t* context)
		:Id(id)
	...
	sof_actor * sof_actor_factory::create(sof_state S,sof_t id,sof_f f, void * userdata,size_t stacksize)
	{
		void * block = __alloc(stacksize + sizeof(sof_actor));
		...
		block = (sof_byte_t*)block - sizeof(sof_actor);

		sof_context_t * context = sof_make_context(block,stacksize,(void(*)(intptr_t))&__f);

		return new (block) sof_actor(id,f,userdata,context);
	}

	void sof_actor_factory::close(sof_actor * actor)
	{
		assert(actor->Context);

		void * block  = (sof_byte_t*)actor + sizeof(sof_actor);

		__free(block,actor->Context->fc_stack.size + sizeof(sof_actor));
	}
}}